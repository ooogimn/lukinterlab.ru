...
**Перенос на другой сайт и связка с блогом:** см. **[README_INTEGRATION.md](README_INTEGRATION.md)** — там описана **триада** `Assistant` + `Moderation` + **`Blog`** (блог как третий неотъемлемый элемент; на новом проекте чаще **подменяют** свою папку блога на эталонную `Blog/` из репозитория и дорабатывают код).
